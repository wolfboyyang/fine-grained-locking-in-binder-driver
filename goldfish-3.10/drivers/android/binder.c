...
static int binder_inc_node_nilocked(struct binder_node *node, int strong, int internal,
			   struct list_head *target_list)
{
	struct binder_proc *proc = node->proc;
...
static struct binder_ref *binder_get_ref_for_node(struct binder_proc *proc,
						  struct binder_node *node, struct binder_ref *new_ref)
{
...
static void binder_cleanup_ref(struct binder_ref *ref)
{
	...
	else binder_free_node(ref->node);
	...
	if (strong) {
		if (ref->data.strong == 0) {
			binder_user_error("%d invalid dec strong, ref %d desc %d s %d w %d\n",
					  ...
					  ref->data.desc, ref->data.strong, ref->data.weak);
			return false;
		}
		ref->data.strong--;
		if (ref->data.strong == 0)
	...
	while (1) {
		...
		if (target_thread) {
			...
			if (target_thread->reply_error.cmd == BR_OK) {
				...
				binder_enqueue_work(target_thread->proc, &target_thread->reply_error.work, &target_thread->todo);
...
err_no_context_mgr_node:
...

	{
		...
		*fe = *e;
		...
		smp_wmb();
		e->debug_id_done = t_debug_id;
		fe->debug_id_done = t_debug_id;
	}

	BUG_ON(thread->return_error.cmd != BR_OK);
	if (in_reply_to) {
		thread->return_error.cmd = BR_TRANSACTION_COMPLETE;
		binder_enqueue_work(thread->proc, &thread->return_error.work, &thread->todo);
			...
			if (cmd == BC_REQUEST_DEATH_NOTIFICATION) {
				...
				if (ref->node->proc == NULL) {
					...
					if (thread->looper & (BINDER_LOOPER_STATE_REGISTERED | BINDER_LOOPER_STATE_ENTERED))
						binder_enqueue_work(proc, &ref->death->work, &thread->todo);
					else {
						binder_enqueue_work(proc, &ref->death->work, &proc->todo);
				...
				if (list_empty(&death->work.entry)) {
					...
					if (thread->looper & (BINDER_LOOPER_STATE_REGISTERED | BINDER_LOOPER_STATE_ENTERED)) {
						binder_enqueue_work_ilocked(&death->work, &thread->todo);
					} else {
...
static int binder_has_proc_work(struct binder_proc *proc,
				struct binder_thread *thread)
{
	return !binder_worklist_empty(proc, &proc->todo) || thread->looper_need_return;
}

static int binder_has_thread_work(struct binder_thread *thread)
{
	return !binder_worklist_empty(thread->proc, &thread->todo) || thread->looper_need_return;
}
...
static int binder_open(struct inode *nodp, struct file *filp)
{
	...
	proc->alloc.binder_alloc_mmap_lock = &binder_dev->context.binder_alloc_mmap_lock;
...
static void print_binder_node_nlocked(struct seq_file *m, struct binder_node *node)
{
	struct binder_ref *ref;
	struct binder_work *w;
	int count;
...
static int binder_state_show(struct seq_file *m, void *unused)
{
	...
	hlist_for_each_entry(device, &binder_devices, hlist) {
		...
		hlist_for_each_entry(node, &context->binder_dead_nodes, dead_node) {
			/*
					...
					struct binder_transaction_log_entry *e)
{
	 ...
	 */
	smp_rmb();
	seq_printf(m, debug_id && debug_id == e->debug_id_done ?
			"\n" : " (incomplete)\n");
}
...
static int __init init_binder_device(const char *name)
{
	...
	atomic_set(&context->transaction_log.cur, ~0U);
	atomic_set(&context->transaction_log_failed.cur, ~0U);
	return ret;
