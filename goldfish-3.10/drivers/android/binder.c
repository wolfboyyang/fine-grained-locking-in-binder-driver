 ...
 * buffers. It is normally initialized during binder_init() and binder_mma
						  ...
						  uintptr_t user_ptr)
{
	...
	kern_ptr = (struct binder_buffer *)(user_ptr - proc->alloc.user_buffer_offset
				    ...
				    struct vm_area_struct *vma)
{
	...
	for (page_addr = start; page_addr < end; page_addr += PAGE_SIZE) {
		...
		page = &proc->alloc.pages[(page_addr - proc->alloc.buffer) / PAGE_SIZE];
	...
	for (page_addr = end - PAGE_SIZE; page_addr >= start;
		...
		if (vma)
			zap_page_range(vma, (uintptr_t)page_addr +
				proc->alloc.user_buffer_offset, PAGE_SIZE, NULL);
					      ...
					      int is_async)
{
	...
	if (is_async &&
	    proc->alloc.free_async_space < size + sizeof(struct binder_buffer)) {
		binder_debug(BINDER_DEBUG_BUFFER_ALLOC,
...
static int binder_mmap(struct file *filp, struct vm_area_struct *vma)
{
	...
	proc->alloc.user_buffer_offset = vma->vm_start - (uintptr_t)proc->alloc.buffer;
...
#endif
	proc->alloc.pages = kzalloc(sizeof(proc->alloc.pages[0]) * ((vma->vm_end - vma->vm_start) / PAGE_SIZE), GFP_KERNEL);
			      ...
			      struct binder_proc *proc, int print_all)
{
	...
	for (n = rb_first(&proc->alloc.allocated_buffers); n != NULL; n = rb_next(n))
