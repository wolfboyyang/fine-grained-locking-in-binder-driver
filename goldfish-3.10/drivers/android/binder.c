				 ...
				 struct binder_buffer *buffer)
{
	if (list_is_last(&buffer->entry, &alloc->buffers))
		return alloc->buffer + alloc->buffer_size - (void *)buffer->data;
...
static struct binder_buffer *binder_alloc_buffer_lookup_locked(struct binder_alloc *alloc,
						  uintptr_t user_ptr)
{
			...
			binder_size_t *consumed)
{
	...
	while (ptr < end && thread->return_error == BR_OK) {
		...
		case BC_FREE_BUFFER: {
			...
			buffer = binder_alloc_buffer_lookup(&proc->alloc, data_ptr);
			if (buffer == NULL) {
			      ...
			      struct vm_area_struct *vma)
{
	...
	mutex_lock(&alloc->context->binder_alloc_mmap_lock);
	if (alloc->buffer) {
		ret = -EBUSY;
	...
	mutex_unlock(&alloc->context->binder_alloc_mmap_lock);
...
static int binder_open(struct inode *nodp, struct file *filp)
{
	...
	proc->alloc.context = &binder_dev->context;
