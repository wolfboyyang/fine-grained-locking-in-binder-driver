...
static int binder_inc_node_ilocked(struct binder_node *node, int strong, int internal,
			   struct list_head *target_list)
{
	//if (node->proc)
...
static bool binder_dec_node_ilocked(struct binder_node *node, int strong, int internal)
{
			...
			binder_size_t *consumed)
{
	...
	while (ptr < end && thread->return_error.cmd == BR_OK) {
		...
		case BC_ACQUIRE_DONE: {
			...
			//binder_inner_proc_lock(node);
			if (cmd == BC_ACQUIRE_DONE) {
				if (node->pending_strong_ref == 0) {
					binder_user_error("%d:%d BC_ACQUIRE_DONE node %d has no pending acquire request\n",
						proc->pid, thread->pid,
						node->debug_id);
					//binder_inner_proc_unlock(node);
					binder_put_node(node);
					break;
				}
